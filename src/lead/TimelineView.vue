<template>
  <div class="min-h-full flex flex-col gap-6 animate-fade-up">
    ...
    <div class="flex-1 min-h-0 grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 bg-white rounded-[2rem] shadow-sm border border-slate-100 p-6 flex flex-col overflow-hidden">
            ...
            <div class="grid grid-cols-7 flex-1 gap-1.5 overflow-y-auto custom-scrollbar p-3">
                <div v-for="n in startOffset" :key="`empty-${n}`" class="p-4 opacity-0"></div>

                <div v-for="day in daysInMonth" :key="day" 
                     @click="selectedDay = day"
                     class="relative border rounded-2xl p-2 hover:bg-slate-50 hover:border-slate-200 transition-all cursor-pointer flex flex-col gap-1 min-h-[85px]"
                     :class="[
                ...
                >
                    <span class="text-[10px] font-black w-6 h-6 flex items-center justify-center rounded-full"
                          :class="[
                    ...
                    <div class="flex flex-col gap-1.5">
                      <div v-for="event in getEventsForDay(day).slice(0, 4)" :key="event.id" 
                           class="text-[7px] font-bold px-2 py-1 rounded-lg shadow-sm transition-transform hover:scale-105 break-words leading-tight"
        ...
        <div class="flex flex-col gap-8 overflow-y-auto custom-scrollbar pr-1">
            <button @click="showAddModal = true" class="bg-[#5c56f0] text-white p-5 rounded-[1.8rem] shadow-xl shadow-indigo-100 hover:bg-indigo-700 hover:-translate-y-1 transition-all flex items-center justify-between group text-left shrink-0">
                <div>
                    <p class="text-[9px] font-black opacity-40 uppercase tracking-[0.3em] mb-1">หัวหน้าทีม</p>
                    <h3 class="text-md font-black uppercase tracking-tight leading-none">เพิ่มรายการใหม่</h3>
                </div>
                <div class="w-9 h-9 bg-white/20 rounded-xl flex items-center justify-center group-hover:rotate-90 transition-transform">
                    <i class="fas fa-plus text-sm"></i>
                </div>
            </button>

            <div class="bg-white rounded-[3rem] shadow-sm border border-slate-100 p-8 flex flex-col gap-6 flex-1 text-left min-h-0">
                <h3 class="text-[14px] font-black text-slate-800 uppercase tracking-[0.2em] flex items-center gap-3">
                    <div class="w-8 h-8 rounded-full bg-indigo-500 text-white flex items-center justify-center text-xs shadow-lg shadow-indigo-200">
                ...
                <div class="space-y-6 overflow-y-auto custom-scrollbar flex-1 pr-1">
                    <div v-if="sidebarEvents.length === 0" class="text-center py-20 opacity-20">
                      <p class="text-[10px] font-black uppercase tracking-widest mt-4">ไม่มีรายการงานสำหรับวันนี้</p>
                    </div>
                    <div v-for="event in sidebarEvents" :key="event.id" class="flex gap-5 items-center group p-2 rounded-2xl hover:bg-slate-50 transition-all">
                        <div class="flex flex-col items-center min-w-[3.8rem] bg-white rounded-2xl p-3 border border-slate-100 text-center shadow-lg group-hover:scale-105 transition-transform">
                            <span class="text-[7px] font-black text-slate-300 uppercase tracking-widest leading-none mb-1">{{ event.monthLabel }}</span>
                            <span class="text-2xl font-black text-slate-800 leading-none">{{ event.day }}</span>
                        </div>
                        <div class="flex-1 min-w-0">
                            <h4 class="text-[13px] font-black text-slate-800 leading-tight mb-1">{{ event.title }}</h4>
                            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-wide">{{ event.time || '10:00 AM' }}</p>
                            <span class="inline-block mt-2 text-[8px] font-black px-2 py-1 rounded-lg uppercase tracking-wider bg-indigo-50 text-indigo-500 shadow-sm">
        ...
        <div class="bg-white w-full max-w-[440px] rounded-[3.5rem] p-12 shadow-2xl animate-pop text-left">
          ...
          <div class="space-y-8">
            ...
            <div class="flex gap-4 pt-4">
              ...
              <button @click="saveNewTask" class="flex-1 py-5 bg-[#5c56f0] text-white rounded-[2rem] font-black uppercase text-[11px] tracking-widest hover:bg-indigo-600 shadow-xl shadow-indigo-100">บันทึกข้อมูล</button>
            </div>
          </div>
        </div>
      </div>
...
const getEventColorClass = (t) => {
  ...
  if (t.type === 'project') return 'bg-indigo-100 text-indigo-700 border border-indigo-200'
  if (t.type === 'urgent') return 'bg-rose-100 text-rose-700 border border-rose-200'
...
const saveNewTask = async () => {
  ...
  await api.createTask({ ...newTask.value, type: 'task', status: 'active', department: 'Operations' });
