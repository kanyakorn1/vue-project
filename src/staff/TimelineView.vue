<template>
    ...
    <div class="flex flex-col gap-6 shrink-0">
        <div class="flex items-center justify-between">
            <div class="flex flex-col gap-2 text-left">
                <h2 class="text-4xl font-black text-[#121926] tracking-tight uppercase leading-none">ตารางงานของฉัน</h2>
                <p class="text-[10px] font-bold text-slate-400 tracking-[0.2em] uppercase">รายการภารกิจส่วนบุคคล: {{ monthYearLabel }}</p>
            </div>
        ...
        <div class="flex items-center justify-start">
            <div class="bg-white p-1.5 rounded-full border border-slate-100 shadow-sm hidden md:block">
    ...
    <div class="flex-1 min-h-0 grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div class="lg:col-span-3 bg-white rounded-[3rem] shadow-sm border border-slate-100 p-10 flex flex-col overflow-hidden">
            ...
            <div class="grid grid-cols-7 flex-1 gap-2 overflow-y-auto custom-scrollbar p-6">
                ...
                <div v-for="day in daysInMonth" :key="day" 
                     ...
                     class="relative border rounded-3xl p-4 hover:bg-slate-50 hover:border-slate-200 transition-all cursor-pointer flex flex-col gap-2 min-h-[120px]"
                     :class="[
                ...
                >
                    <span class="text-xs font-black w-8 h-8 flex items-center justify-center rounded-full"
                          :class="[
                            selectedDay === day ? 'bg-[#5c56f0] text-white shadow-md shadow-indigo-200' : (isToday(day) ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-200' : 'text-slate-700'),
        ...
        <div class="flex flex-col gap-8 overflow-y-auto custom-scrollbar pr-1">
            <button @click="showAddModal = true" class="bg-[#121926] text-white p-8 rounded-[2.5rem] shadow-2xl shadow-slate-200 hover:bg-slate-800 hover:-translate-y-1 transition-all flex items-center justify-between group text-left shrink-0">
                <div>
                    <p class="text-[10px] font-black opacity-40 uppercase tracking-[0.3em] mb-1">PERSONAL STAFF</p>
                    <h3 class="text-xl font-black uppercase tracking-tight leading-none">เพิ่มบันทึกช่วยจำ</h3>
                </div>
            ...
            <div class="bg-white rounded-[3rem] shadow-sm border border-slate-100 p-8 flex flex-col gap-6 flex-1 text-left min-h-0">
                <h3 class="text-sm font-black text-slate-800 uppercase tracking-widest flex items-center gap-3">
                    <i class="fas fa-user-clock text-indigo-500"></i> กำหนดการวันนี้
                </h3>
                ...
                <div class="space-y-6 overflow-y-auto custom-scrollbar flex-1 pr-1">
                    <div v-if="sidebarEvents.length === 0" class="text-center py-20 opacity-20">
                      <p class="text-[10px] font-black uppercase tracking-widest text-center">ไม่มีภารกิจสำหรับวันนี้</p>
                    </div>
                    <div v-for="event in sidebarEvents" :key="event.id" class="flex gap-4 items-center group">
    ...
    <transition name="fade">
      <div v-if="showAddModal" class="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-6">
        <div class="bg-white w-full max-w-[440px] rounded-[3.5rem] p-12 shadow-2xl animate-pop text-left">
          <h2 class="text-3xl font-black text-slate-800 tracking-tighter mb-2 uppercase italic leading-none">เพิ่มบันทึกช่วยจำ</h2>
          <p class="text-slate-400 text-[11px] font-bold uppercase tracking-widest mb-10">สร้างภารกิจส่วนตัว</p>
          <div class="space-y-8">
            <div class="space-y-3">
            ...
            <div class="flex gap-4 pt-4">
              ...
              <button @click="saveNewTask" class="flex-1 py-5 bg-indigo-600 text-white rounded-[2rem] font-black uppercase text-[11px] tracking-widest shadow-xl shadow-indigo-100">บันทึกช่วยจำ</button>
            </div>
          </div>
        </div>
      </div>
  ...
  const isTaskCritical = (task) => {
    ...
    // For staff: Urgent status or Overdue
  ...
  const getEventColorClass = (t) => {
    ...
    if (t.status === 'urgent') return 'bg-rose-100 text-rose-600';
    if (t.type === 'project') return 'bg-indigo-100 text-indigo-600';
  ...
  const saveNewTask = async () => {
    ...
    await api.createTask({ ...newTask.value, type: 'task', status: 'active', assignedTo: props.userEmail });
